...
class CommentBlockMatcher {
  ...
  match(comments) {
    ...
    const isArray = Array.isArray(this.expectedLines);
    let expectedLines = isArray
      ? this.expectedLines.map((line) =>      `${this.linePrefix}${line}`.trimEnd())
      : this.expectedLines;
    if (this.style === "jsdoc") {
    ...
    if(isArray) {
      bodyMatch= expectedLines.every((line) => {
      ...
      bodyMatch = normalizedComments[0].value.trim() === expectedLines
    }
