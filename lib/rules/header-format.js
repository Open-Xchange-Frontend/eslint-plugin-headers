 ...
 * @returns {boolean} Flag indicating the AST should be treated as Vue
 */
// function parseAsVue(style) {
//   return context.
// }
 ...
 * @returns Node
 */
function getMissingHeaderInsertionPointNode(context, program) {
  if (parseAsVue(context)) {
...
module.exports = {
  ...
  create(context) {
    ...
    const expectedHeaderLines = normalizedHeaderText;
    ...
    return {
      Program: function (node) {
        if (!hasHeaderComment(node, context)) {
          const insertionPoint = getMissingHeaderInsertionPointNode(
            context,
            node,
          );
          context.report({
